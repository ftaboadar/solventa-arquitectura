...
**No cambia el diseño ni el código.** El puerto interno de cada servicio se fija a `8080` (convención de Cloud Run). Para `acl-worker` y `consumidor-under` se fija vía sus propias env vars (`ACL_WORKER_PORT`, `UNDER_PORT`); `stub-kyc` no necesita ninguna porque ya lee `os.environ["PORT"]`, y `PORT` es un nombre reservado que Cloud Run inyecta solo — fijarlo manualmente hace fallar el despliegue (`400: reserved env names were provided: PORT`).

**Nota (2026-09-13): las 3 piezas se migraron de Node.js a Python** (ver `DISENO-EXPERIMENTOS.md`, "Resultados y análisis" del Experimento 1). Este Terraform no cambia por eso: no referencia el runtime de la imagen (Cloud Run solo necesita un contenedor Linux escuchando en `PORT`/`8080`), así que sigue funcionando igual con las imágenes Python — la única diferencia real es qué construye `scripts/build-and-push.sh` a partir de cada `Dockerfile`. **`terraform apply` sigue sin haberse ejecutado contra las imágenes Python** — habría que repetir la Fase 2 (rebuild y push) y la Fase 3 (`terraform apply`) si se quiere validar el despliegue real en Cloud Run con el nuevo stack; no se hizo en esta migración porque no estaba pedido y evita gasto de facturación no solicitado explícitamente.
